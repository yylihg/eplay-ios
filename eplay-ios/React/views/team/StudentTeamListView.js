...
export default class StudentTeamListView extends Component {
    ...
    _getList = function (page) {
        var url = "/answerBuy/list.do?start="+page*10+"&pageSize=10";
        var context = this;
        ReactModule.fetch("get",{api:url},function (error, response) {
            console.log('ihg /answerBuy/list.do', response)
            if (error){
                currentPage -=1;
            }else {
    ...
    _onItemPress(item) {
        ...
        ReactModule.fetch("get", {api: "/answerClass/goAnswerClass.do?orderId=" + item.ANSWER_ORDER_ID}, function (error, response) {
            console.log('ihg /answerClass/goAnswerClass.do', response)
