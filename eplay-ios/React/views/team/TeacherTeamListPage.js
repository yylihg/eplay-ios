...
export default class TeacherTeamListPage extends Component {
    ...
    _getList = function (page) {
        var url = "/answerCourse/list.do?start="+page*10+"&pageSize=10&userId=" + this.state.teacherId;
        var context = this;
        ReactModule.fetch("get",{api:url},function (error, response) {
            console.log('ihg /answerCourse/list.do', response)
            if (error){
                currentPage -=1;
            }else {
    ...
    render() {
        return (
            <View>
                <HeadViewWithLeftBtn title = "小组课"></HeadViewWithLeftBtn>
                <AnimatedFlatList
    ...
    _onItemPress(item) {
        ReactModule.fetch("get", {api: "/answerClass/beginClassNotice.do?orderId=" + item.ORDER_ID}, function (error, response) {
            console.log('ihg /answerClass/beginClassNotice.do', response)
            if (error) {
            } else {
            }
        });
        ReactModule.fetch("get", {api: "/answerClass/begin.do?orderId=" + item.ORDER_ID}, function (error, response) {
