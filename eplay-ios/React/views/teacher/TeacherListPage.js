...
class TeacherListPage extends Component {
    ...
    _getTeacherList = function (page) {
        var context = this;
        ReactModule.fetch("get",{api:this.props.api +  "start="+page*10+"&pageSize=10"},function (error, response) {
            console.log('ihg /teacher/list.do: ' + page, response)
            if (error){
                currentPage -=1;
            }else {
    ...
    render() {
        return (
            <View>
                <HeadViewWithLeftBtn title = "教师列表"></HeadViewWithLeftBtn>
                <AnimatedFlatList
