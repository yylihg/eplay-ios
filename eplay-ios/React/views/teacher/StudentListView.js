...
class StudentListPage extends Component {
    ...
    _getVideoList = function (page) {
        var context = this;
        ReactModule.fetch("get",{api:this.props.api +  "?start="+page*10+"&pageSize=10" + "&teacherUserId=" + this.props.id},function (error, response) {
            console.log('ihg ' + context.props.api, response)
            if (error){
                currentPage -=1;
            }else {
    ...
    _onItemPress(item) {
        ReactModule.pushVideoViewController(findNodeHandle(this), "/studentVideo/getVideoUrl.do?videoId="+ item.VIDEO_ID, item.VIDEO_NAME);
    }
