...
export default class StudentGroupListPage extends Component {
    ...
    _getList = function (page) {
        var url = "/groupBuy/list.do?start="+page*10+"&pageSize=10";
        var context = this;
        ReactModule.fetch("get",{api:url},function (error, response) {
            console.log('ihg groupBuy/list.do', response)
            if (error){
                currentPage -=1;
            }else {
    ...
    render() {
        return (
            <View>
                <HeadViewWithLeftBtn title = "我的团购课程"></HeadViewWithLeftBtn>
                <AnimatedFlatList
    ...
    _onItemPress(item) {
        ReactModule.pushReactViewController(findNodeHandle(this), "StudentGroupBuyDetailPage", item);
    }
